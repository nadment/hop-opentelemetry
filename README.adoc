...
2. Configure OpenTelemetry manually by setting the system variables below or using the GUI.
		OTEL_SERVICE_NAME: Set the default service name.
		OTEL_EXPORTER_OTLP_ENDPOINT: Set target URL of the OpenTelemetry Collector to which send spans, metrics.
3. Restart Hop
